...
export const findPaginatedByCampaignId = async ({
  ...
  const { results, pagination } = await fetchPage(
    getParticipantsResultList(campaignId, stageCollection, filters),
    page,
  );
...
const getParticipantsResultList = (campaignId, stageCollection, filters) =>
  knex
    .with('campaign_participation_summaries', (qb) => getParticipations(qb, campaignId, stageCollection, filters))
    .select('*')
...
const getParticipations = (qb, campaignId, stageCollection, filters) => {
  qb.select(
...
const getAcquiredStages = async (campaignParticipationId) =>
  await knex(STAGE_ACQUISITIONS_TABLE_NAME).select('*').where({ campaignParticipationId: campaignParticipationId });
